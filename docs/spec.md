...
- `/resume` renders from `src/data/resume.ts` with role DevOps / Platform Engineer, a short systems-practice summary (not years of employment), and skills including MinIO, Argo CD, and Loki. The HTML page does not repeat the contact line; header and footer are the site chrome, and the PDF carries phone plus the full ATS contact row. A 2021→platform path may appear on the HTML page only; it wraps after freelance as two nowrap lines with `→ …` / `… →` so a step is never split mid-token. It must not appear in the PDF.
